...
import { ERC20Token, useErc20List, useHinkalContext } from '@hinkal/react-hooks';
...
export const TokenAmountInput = ({
   ...
   */
  const setTokenAmountHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="flex flex-col item-center justify-center">
      ...
      <div className={`flex justify-center mt-1 mb-8 ${buttonWrapperStyles} w-[90%] mx-auto relative`}>
        <Listbox
        ...
        >
          {({ open }) => (
            <>
              ...
              >
                <img src={selectedToken.logoURI} alt="tokenIcon" className="w-[26px]" />
              ...
              <Listbox.Options className="absolute w-full top-10 text-white flex flex-col bg-[#272B30] rounded-b-lg z-20">
                {erc20List.map((token, index) => (
                  ...
                  >
                    <img src={token.logoURI} alt="tokenIcon" className="w-[26px]" /> <span>{token.symbol}</span>
                  </Listbox.Option>
                ))}
              </Listbox.Options>
            </>
